...
class AsteAvvisiSearchSpider(SeleniumSearchSpider):
    
    ''' Copy of canaleaste '''
        
    name = 'asteavvisi'
    allowed_domains = ['www.asteavvisi.it']
    start_urls = [
        'https://www.asteavvisi.it/aste/cerca?tipologia_lotto=immobiliare&limit=36&orderby=datavdesc&tipologia=A&proc_old=true'
    ]

    URL = 'https://www.asteavvisi.it/aste/cerca?tipologia_lotto=immobiliare&limit=36&orderby=datavdesc&tipologia=A&proc_old=true'
    LOGIN_URL = ''
    ...
    SEARCH_XPATH = {
        ...
        'number_of_results' : '//div[@class="container mb-2"]/div[@class="row no-gutters"]/div[@class="col-12 boder-none border-sm-bottom py-3"]/div[@class="row"]/div[@class="col-12 col-sm-7 align-self-center"]/h5[@class="font-weight-bold mb-3 mb-sm-0"]',
        'number_of_results_per_page' : '//div[@class="card vertical-card horizontal rounded overflow-hidden mb-4 border-0 drop-shadow"]',
        #'scroll' : '//li[@class="page-item"]/a[class="page-link nuxt-link-active"][contains(text(), "›")]',
