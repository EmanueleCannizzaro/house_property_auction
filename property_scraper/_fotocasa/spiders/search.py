...
from property_scraper.fotocasa.items import PropertyItem
#from scrapy_splash import SplashRequest


class FotocasaSearchSpider(scrapy.Spider):
    name = 'fotocasa_search'

    pages = 10
    start_urls = [f'https://www.fotocasa.es/es/comprar/viviendas/barcelona-capital/todas-las-zonas/l/{i+1}?sortType=price' for i in range(pages)]
    ...
    def start_requests(self):
        for page in self.start_urls:
            yield scrapy.Request(url=page, headers=self.headers, callback=self.crawl_page)

    def crawl_page(self, response):
        #article class="re-CardPackMinimal"
        # script/text()')[3].root -> this will get the variable for the json
        property_urls = response.xpath('//article[@class="re-CardPackMinimal"]/a/@href')
        print(response.text[:1000])
        print(property_urls)
        #href="/es/comprar/vivienda/barcelona-capital/aire-acondicionado-terraza-se-aceptan-mascotas-no-amueblado/163467609/d?from=list"
        for property_url in property_urls:
            url = 'https://www.fotocasa.es/' + property_url
            yield scrapy.Request(url=url, headers=self.headers, callback=self.crawl_property)
...
if __name__ == '__main__':
    ...
    process.crawl(FotocasaSearchSpider)
    process.start()
