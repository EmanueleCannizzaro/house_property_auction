...
#from scrapy_rotated_proxy.downloadmiddlewares.proxy import RotatedProxyMiddleware
...
#class PvpDownloaderMiddleware(RotatedProxyMiddleware):
class PvpDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
    # scrapy acts as if the downloader middleware does not modify the
    # passed objects.
...
class HtmlMiddleware(object):
    ...
    def _save_html_to(self, path, html_body):
        ...
        """
        fs.write_to_file(path, html_body, self.gzip_output)
    ...
    def _should_save_html(self, request, response):
        ...
        return 'save_html' in request.meta and \
            self.should_save_html_according_response_code(
                response.status,
    ...
    @staticmethod
    def should_save_html_according_response_code(code:int, allowed_list:list):
        """
        Args:
            code : response status code
            allowed_list : list of response status codes allowed to save html
        Returns:
