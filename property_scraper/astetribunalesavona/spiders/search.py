...
@traced
@logged
class AsteTribunaleSavonaSearchSpider(SeleniumSearchSpider):
    name = 'astetribunalesavona'
    allowed_domains = ['www.tribunale.savona.giustizia.it']
    start_urls = ['https://www.tribunale.savona.giustizia.it/it/Aste/SearchImmobili']

    URL = 'https://www.tribunale.savona.giustizia.it/it/Aste/SearchImmobili'
    LOGIN_URL = ''
    #NEXT_PAGE_URL = ''
    WEBSITE = 'astetribunalesavona'
    LOGIN_XPATH = {
    ...
    SEARCH_XPATH = {
        'cookie' : '//button[@class="cookiebar-btn cookiebar-confirm"][contains(text(), "Accetto")]',
        #'url' : '//button[@class="cc-button-search cc-button-search-primary we"][@id="btnSimpleSearchImmobili"][contains(text(), "Cerca")]',
        'number_of_results' : '//div[@class="col-4"]/small[contains(text(), "Trovati ")]',
        'number_of_results_per_page' : '//div[@class="card card-bg card-img card-space no-after"]',
        'scroll' : '//li[@class="page-item"]/a[@class="app-btnNextPage page-link"]/span[@class="sr-only"][contains(text(), "Pagina successiva")]/ancestor::a',
    }
    SEARCH_SELECT = {
        ...
        #'//ul[@class="dropdown-content select-dropdown"]/li/span[contains(text(), "120")]'
    ...
    def get_number_of_results(element):
        return element.text.replace('Trovati ', '').strip().split(' ')[0]
