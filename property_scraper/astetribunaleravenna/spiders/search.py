...
@traced
@logged
class AsteTribunaleRavennaSearchSpider(SeleniumSearchSpider):
    name = 'astetribunaleravenna'
    allowed_domains = ['www.asteravenna.eu']
    start_urls = ['https://www.asteravenna.eu']

    URL = 'https://www.asteravenna.eu'
    LOGIN_URL = ''
    NEXT_PAGE_URL = 'https://www.asteravenna.eu/?page={}'
    WEBSITE = 'astetribunaleravenna'
    LOGIN_XPATH = {
    ...
    SEARCH_XPATH = {
        ...
        'number_of_results' : '//h4[@class="font-weight-bold"][contains(text(), " Aste Giudiziarie")]',
        'number_of_results_per_page' : '//div[@class="card vertical-card horizontal rounded overflow-hidden mb-4 border-0 drop-shadow"]',
        #'scroll' : '//div[@class="container"]/div[@class="row"]/div[@class="col-10 py-2 py-sm-0 pt-sm-2 pagination_tribunali"]/nav[@class="mb-0 float-right"]/ul[@class="pagination b-pagination justify-content-end"]/li[@class="page-item"][position()=8]/a[class="page-link"]',
    }
    SEARCH_SELECT = {
    ...
    def get_number_of_results(element):
        return element.text.replace('n. ', '').strip().split(' ')[0]
