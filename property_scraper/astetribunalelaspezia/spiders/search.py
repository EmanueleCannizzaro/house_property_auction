...
@traced
@logged
class AsteTribunaleLaspeziaSearchSpider(SeleniumSearchSpider):
    name = 'astetribunalelaspezia'
    allowed_domains = ['www.tribunale.laspezia.it']
    start_urls = ['https://www.tribunale.laspezia.it/it/Aste/SearchImmobile']

    URL = 'https://www.tribunale.laspezia.it/it/Aste/SearchImmobile'
    LOGIN_URL = ''
    #NEXT_PAGE_URL = ''
    WEBSITE = 'astetribunalelaspezia'
    LOGIN_XPATH = {
    ...
    SEARCH_XPATH = {
        ...
        'number_of_results' : '//div[contains(text(), "La ricerca ha prodotto ")]',
        'number_of_results_per_page' : '//div[@class="asta"]',
        'scroll' : '//div[@class="pager pagerPVP"]/a[@class="paging"][contains(text(), "Successiva")]',
    }
    SEARCH_SELECT = {
        ...
        'number_of_results_per_page' : '//select[@class="OrderBy_ddlRp"]',
    ...
    def get_number_of_results(element):
        return element.text.replace("La ricerca ha prodotto ", "").strip().split(' ')[0]
