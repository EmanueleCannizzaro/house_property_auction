...
from property_scraper import DEFAULT_GEOCODE_ENGINE, DEFAULT_HTML_PARSER, IMMOBILIARE_URL_ROOTNAME
from property_scraper.immobiliare_page import ImmobiliarePage
from property_scraper.scraper import Scraper


@logged(logging.getLogger("property_scraper"))
class ImmobiliareScraper(Scraper):
    """The `Immobiliare` webscraper collects structured data on properties
    ...
    NUMBER_OF_RESULTS = {
        'tag' : 'div', 
        'attribute': 'class',
        'value': 'in-searchList__title'
    }


    def __init__(self, name:str=None):
        super(ImmobiliareScraper, self).__init__()
        self.page = ImmobiliarePage()
        
        self._property_type = self.PROPERTY_TYPES[1]


    def set_url(self, parameters:dict):        
        _url = f"{self.URL_ROOTNAME}/vendita-case/{parameters['citta'].lower()}/?"
        count = 0
