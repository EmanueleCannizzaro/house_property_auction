...
from property_scraper.recrowd.items import RecrowdMyInvestmentItem, RecrowdMyInvestmentModel
#from property_scraper.pvp.pipelines import get_location


@traced
@logged
class RecrowdMyInvestmentSpider(SeleniumSearchSpider):
    name = 'recrowd_investiment'
    allowed_domains = ['www.recrowd.com']
    start_urls = [
        'https://www.recrowd.com/it/area-privata/investimenti'
    ]
    search_datetime = datetime.now().strftime("%Y%m%d%H%M%S")

    URL = 'https://www.recrowd.com/it/area-privata/investimenti'
    ...
    SEARCH_SELECT = {
        'number_of_results_per_page' : '//div[@class="card__text"][contains(text(), "Finanziati")]/preceding-sibling::div[@class="card__amount"]',
...
if __name__ == '__main__':
    ...
    process.crawl(RecrowdMyInvestmentSpider)
