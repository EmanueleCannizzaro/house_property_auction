...
from property_scraper.recrowd.items import RecrowdMyDashboardItem, RecrowdMyDashboardModel
#from property_scraper.pvp.pipelines import get_location


@traced
@logged
class RecrowdMyDashboardSpider(SeleniumSearchSpider):
    name = 'recrowd_dashboard'
    allowed_domains = ['www.recrowd.com']
    start_urls = [
        'https://www.recrowd.com/it/area-privata/dashboard'
    ]
    search_datetime = datetime.now().strftime("%Y%m%d%H%M%S")

    URL = 'https://www.recrowd.com/it/area-privata/dashboard'
...
if __name__ == '__main__':
    ...
    process.crawl(RecrowdMyDashboardSpider)
