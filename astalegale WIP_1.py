...
# Date : 8/10/2017
# Description : web crawler for https://www.astalegale.net
#
# Requirements:
# - selenium (run "pip install selenium" from cmd)
# - Mozilla Firefox
# - geckodriver.exe, to be placed in PATH 
#
# To Launch:
...
# global vars
#                   Type, Price Min, Price Max, Region, Province, Town
search_params = ["Appartamento", 80000, 500000, "Veneto", "Vicenza", "Schio"]
links = []
...
def init_driver(path_geckodriver="/Users/Andrea/geckodriver"):
...
def lookup(driver, search_params):
    ...
    try:
        boxRegion = Select(driver.wait.until(EC.presence_of_element_located(
            (By.ID, "select-options-1cb99b8e-87dc-2cb8-956e-4bc471f98d3c")))) #Regione

        boxRegion = Select(driver.wait.until(EC.presence_of_element_located(
            (By.CSS_SELECTOR, "ddlRegione")))) #Regione

        boxProvince = Select(driver.wait.until(EC.presence_of_element_located(
            (By.CSS_SELECTOR, "select-options-dcad387e-5a27-0baa-3ff0-d6e9626b3020"))))
        boxTown = driver.wait.until(EC.presence_of_element_located((By.XPATH, "ddlComune")))


        boxRegion = driver.wait.until(EC.presence_of_element_located((By.NAME, "RegioneId")))
        boxProvince.select_by_visible_text("Vicenza")


        boxProvince = driver.wait.until(EC.presence_of_element_located((By.NAME, "SelectedRegioneId")))
        boxProvince.select_by_visible_text("Vicenza")
        ...
        boxType = Select(driver.wait.until(EC.presence_of_element_located(
            (By.CSS_SELECTOR, "select-options-1cb99b8e-87dc-2cb8-956e-4bc471f98d3c")))) #Regione
        boxPriceFrom = driver.wait.until(EC.presence_of_element_located(
            (By.XPATH, """//*[@id="ctl00_ContentPlaceHolder1_Mascherericerche1_ImmobiliareGenerale1_txtFasciaPrezzoDa"]""")))
        boxPriceTo = driver.wait.until(EC.presence_of_element_located(
            (By.XPATH, """//*[@id="ctl00_ContentPlaceHolder1_Mascherericerche1_ImmobiliareGenerale1_txtFasciaPrezzoA"]""")))
        ...
        boxRegion.select_by_visible_text(search_params[3])
        boxType.select_by_visible_text(search_params[0])
        boxPriceFrom.send_keys(search_params[1])
        boxPriceTo.send_keys(search_params[2])
        boxProvince.select_by_visible_text(search_params[4])
        boxTown.send_keys(search_params[5])
...
def getlinks(driver):
    ...
    while pages_remaining:
        ...
        for a in driver.find_elements_by_class_name('schedadettagliata'):
...
def readhtml(url):
    ...
    headerarr = [re.sub(r'\b1\b','',w) for w in headerarr] #eliminate suffix 1
...
if __name__ == "__main__":
    driver = init_driver("/Users/Andrea/geckodriver")
    lookup(driver, search_params)
    getlinks(driver)
    data = looplinks(driver)
    driver.quit()
    outfileName = "astegiudiziarie_" + search_params[4] + "_da" + str(search_params[1]) + "_a" + str(search_params[2]) + ".txt"
